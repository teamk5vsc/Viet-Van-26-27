...
- **Model dự phòng**: Tự động chuyển đổi nếu model hiện tại gặp lỗi/quá tải (xem `MODEL_FALLBACK_CHAIN` trong `server.ts`):
  1. `gemini-flash-latest`
  2. `gemini-pro-latest`
  3. `gemini-2.5-flash`
  4. `gemini-2.5-pro`
- **Lưu ý**: các id dạng `gemini-3-*-preview` và `gemini-2.0-flash` từng dùng trước đây đã bị Google
  ngừng hỗ trợ (xác nhận qua lỗi 404 thực tế). Ưu tiên dùng alias `-latest` (tự động trỏ tới bản mới
  nhất Google khuyến nghị) để tránh phải sửa code mỗi khi Google đổi tên/khai tử phiên bản model.
- **Cơ chế Retry**:
...
- **Giao diện**:
  - **Thiết lập Model & API Key**: Cần hiển thị như hình mẫu.
    ...
    - Thứ tự hiển thị: `gemini-flash-latest` (Default), `gemini-pro-latest`, `gemini-2.5-flash`.
